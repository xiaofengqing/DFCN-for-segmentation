...
class DenseLayer_1(nn.Sequential):
    def __init__(self, in_channels, growth_rate):
        super(DenseLayer_1, self).__init__()
        self.add_module('norm', nn.BatchNorm2d(num_features=in_channels))
        self.add_module('relu', nn.ReLU(inplace=True))
        self.add_module('conv', nn.Conv2d(in_channels=in_channels,
                                          out_channels=growth_rate, kernel_size=3, stride=1,dilation=2,
                                          padding=2, bias=True))

        self.add_module('drop', nn.Dropout2d(0.2))

    def forward(self, x):
        return super(DenseLayer_1, self).forward(x)
...
class DenseBlock_1(nn.Module):
    def __init__(self, in_channels, growth_rate, n_layers, upsample=False):
        ...
        self.upsample = upsample
        self.layers = nn.ModuleList([DenseLayer_1(
            in_channels + i * growth_rate, growth_rate)
            for i in range(n_layers)])
...
class TransitionDown_1(nn.Sequential):
    def __init__(self, in_channels):
        super(TransitionDown_1, self).__init__()
        self.add_module('norm', nn.BatchNorm2d(num_features=in_channels))
        self.add_module('relu', nn.ReLU(inplace=True))
        self.add_module('conv', nn.Conv2d(in_channels=in_channels,
                                          out_channels=in_channels, kernel_size=1, stride=1,
                                          padding=0, bias=True))
        self.add_module('drop', nn.Dropout2d(0.2))
        self.add_module('maxpool', nn.MaxPool2d(2,stride=1,padding=1))

    def forward(self, x):
        return super(TransitionDown_1, self).forward(x)
...
class TransitionUp_1(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(TransitionUp_1, self).__init__()
        self.convTrans = nn.ConvTranspose2d(in_channels=in_channels,
...
class Bottleneck_1(nn.Sequential):
    def __init__(self, in_channels, growth_rate, n_layers):
        super(Bottleneck_1, self).__init__()
        self.add_module('bottleneck', DenseBlock_1(in_channels, growth_rate, n_layers, upsample=True))

    def forward(self, x):
        return super(Bottleneck_1, self).forward(x)


class FCDenseNet(nn.Module):
    def __init__(self, in_channels=3, down_blocks=(5, 5, 5, 5, 5),
                 up_blocks=(5, 5, 5, 5, 5), bottleneck_layers=5,
                 growth_rate=16, out_chans_first_conv=48, n_classes=6):
        ...
        for i in range(len(down_blocks)):
            ...
            if i < len(down_blocks)-1:
                ...
                self.transDownBlocks.append(TransitionDown_1(cur_channels_count))

        #####################
        #     Bottleneck    #
        #####################

        self.add_module('bottleneck', Bottleneck_1(cur_channels_count,
        ...
        for i in range(len(up_blocks) - 1):
            if i == 0:
                self.transUpBlocks.append(TransitionUp_1(prev_block_channels, prev_block_channels))
