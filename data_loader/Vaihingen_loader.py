...
class VaihingenLoader(data.Dataset):
    def __init__(self, root, split="train1", is_transform=False, img_size=256):
        self.root = root
        self.split = split
        ...
        for split in ["train1", "validation", ]:
            file_list = os.listdir(self.root + 'image_data/'+ split + '/' + 'label/')
            self.files[split] = file_list

        if not os.path.isdir(self.root + '/image_data/train1/pre_encoded'):
    ...
    def __getitem__(self, index):
        ...
        lbl_path = self.root + '/image_data/train1/' + 'pre_encoded/' + img_name[:-4] + '.png'
    ...
    def get_pascal_labels(self):
        return np.asarray([[255, 255, 255], [0, 0, 255], [0, 255, 255], [0, 255, 0], [255, 255, 0]])
    ...
    def setup(self, pre_encode=False):
        target_path = self.root + '/image_data/train1/pre_encoded/'
        if not os.path.exists(target_path):
            os.makedirs(target_path)
        if pre_encode:
            print("Pre-encoding segmentation masks...")
            for i in tqdm(self.files['train1']):
                lbl_path = self.root+ '/image_data/train1/label/' + i
...
if __name__ == '__main__':
    local_path = '/data/Vaihingen/'
    dst = VaihingenLoader(local_path, is_transform=True)
